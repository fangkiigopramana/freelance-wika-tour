    ...
    <div class="flex h-[calc(100svh-80px)] ">
        <div class="w-72 min-w-72 bg-white h-full py-8 px-4 flex flex-col font-bold gap-3">
            ...
            <a href="{{ route("pesanan.index") }}">
                <div class="h-10 flex items-center py-4 px-4 gap-2.5 cursor-pointer hover:bg-blue-100 rounded-lg">
                    <img src="{{ asset("assets/group.svg") }}" alt="users icon">
                    <p class=" text-sm text-themeGray ">
                        Data Pemsanan</p>
                </div>
            </a>

            <a href="{{ route("user.index") }}">
                <div class="h-10 flex items-center py-4 px-4 ml-4 gap-2.5 bg-blue-100 rounded-lg cursor-pointer">
                    <img src="{{ asset("assets/article.svg") }}" alt="users icon">
                    <p class="text-sm">
                        Data Pelanggan</p>
                </div>
            </a>

            <form action="{{ route("logout") }}" class="mt-auto flex items-center py-2 px-2 gap-2 font-thin"
                method="POST">
                @csrf
